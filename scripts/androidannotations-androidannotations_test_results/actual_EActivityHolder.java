...
public class EActivityHolder extends EComponentWithViewSupportHolder implements HasIntentBuilder, HasExtras, HasInstanceState, HasOptionsMenu, HasOnActivityResult, HasReceiverRegistration {
    ...
    private void handleBackPressed() {
        ...
        if (declaredOnBackPressedMethod != null) {
            ...
            //
            onKeyDownBody._if(//
            sdkInt.lt(JExpr.lit(5)).cand(//
            keyCodeParam.eq(keyEventClass.staticRef("KEYCODE_BACK"))).cand(//
            eventParam.invoke("getRepeatCount").eq(JExpr.lit(0))))._then().invoke("onBackPressed");
            //
            onKeyDownBody._return(//
            JExpr._super().invoke(onKeyDownMethod).arg(//
            keyCodeParam).arg(eventParam));
        }
    }
