...
echo ",Conflicts Found,Conflicts Resolved,Perfect Resolutions, Perfect w/o Comments,% Conflicts Resolved,% Perfect Resolutions, % Perfect No Comments" > $REPO_$3.csv
printf "%s,%d,%d,%d,%d,%d,%d,%d" "$REPO" "$CONFLICTS" "$SUCCESSES" "$PERFECT_MERGES" "$PERFECT_NO_COMMENTS" "$PERCENT_RESOLVED" "$PERCENT_PERFECT" "$PERCENT_PERFECT_NO_COMMENTS" >> $REPO_$3.csv
